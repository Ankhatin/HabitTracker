...
def set_tasks():
    ...
    for habit in habit_list:
        ...
        if days_since_last == 0:
            start_time = datetime.combine(current_date, time_of_action)
            if current_time.time() > time_of_action:
            ...
            start_time = datetime.combine(current_date, time_of_action) + timedelta(
                days=days_to_action
            )
...
def set_periodic_task(pk, time):
    """
    Метод создает и созраняет в базе данных периодическую задачу, для последующего её исполнения
    ...
    if habit.owner.tg_id:
        ...
        PeriodicTask.objects.create(
            ...
            task="good_habit.tasks.send_notification",
            kwargs=json.dumps(
                {
